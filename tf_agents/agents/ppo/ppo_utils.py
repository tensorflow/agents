...
def distribution_from_spec(
  ...
  Args:
    ...
    new_distribution_params: Parameters to merge with the spec to create a new
...
def get_distribution_params(
  ...
  def _maybe_scale(d, k):
    ...
    if (
        isinstance(d, tfp.distributions.MultivariateNormalDiag)
        and k == 'scale_diag'
    ):
      return 'scale'
    else:
  ...
  def _tensor_parameters_only(d, params):
    return {
        _maybe_scale(d, k): params[k] for k in params if tf.is_tensor(params[k])
    }

  return tf.nest.map_structure(
      lambda d: _tensor_parameters_only(d, d.parameters), nested_distribution
  )
...
def nested_kl_divergence(
  ...
  all_kl_divergences = [
      from_dist.kl_divergence(to_dist)
      for from_dist, to_dist in zip(
          flat_from_distribution, flat_to_distribution
      )
  ]
