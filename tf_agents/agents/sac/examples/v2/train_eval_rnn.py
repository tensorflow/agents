...
def train_eval(
  ...
  with tf.compat.v2.summary.record_if(
    ...
    env_load_fn = functools.partial(
        suite_dm_control.load, task_name=task_name, env_wrappers=env_wrappers
    )
    ...
    dataset = (
        replay_buffer.as_dataset(
            sample_batch_size=batch_size, num_steps=train_sequence_length + 1
        )
        .unbatch()
    ...
    for _ in range(num_iterations):
      ...
      for _ in range(episode_steps):
        ...
        if global_step.numpy() % log_interval == 0:
          logging.info(
              'env steps = %d, average return = %f',
              env_steps.result(),
              average_return.result(),
          )
          env_steps_per_sec = (
              env_steps.result().numpy() - env_steps_before
          ) / time_acc
