...
class DqnAgentTest(test_utils.TestCase):
  ...
  def testCreateAgentWithArraySpecs(self, agent_class):
    ...
    agent = agent_class(
        time_step_spec, action_spec, q_network=q_net, optimizer=None
    )
    self.assertIsNotNone(agent.policy)
  ...
  def testInvalidNetworkOutputSize(self, agent_class):
    ...
    q_net = q_network.QNetwork(
        self._time_step_spec.observation, wrong_action_spec
    )
  ...
  def testLossWithL2Regularization(self, agent_class):
    q_net = DummyNet(
        self._observation_spec, self._action_spec, l2_regularization_weight=1.0
    )
    agent = agent_class(
        self._time_step_spec, self._action_spec, q_network=q_net, optimizer=None
    )
  ...
  def testPolicy(self, agent_class):
    ...
    # Batch size 2.
    self.assertAllEqual(
        action_step.action.shape, [2] + self._action_spec.shape.as_list()
    )
    self.evaluate(tf.compat.v1.global_variables_initializer())
