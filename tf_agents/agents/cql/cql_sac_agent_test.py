...
class _MockDistribution(object):
  ...
  def log_prob(self, sample):
    return tf.constant(10.0, shape=sample.shape)
...
class CqlSacAgentTest(test_utils.TestCase, parameterized.TestCase):
  ...
  def testCqlLoss(self, cql_alpha, num_cql_samples, expected_loss):
    ...
    loss = (
        agent._cql_loss(time_steps, actions, training=False)
        * agent._get_cql_alpha()
    )
  ...
  @parameterized.parameters(
      (False, 0.0, False, [16.3, 28.1]),
      (True, 0.0, True, [7.3, 19.1]),
