...
class ActorNetworkTest(tf.test.TestCase):
  ...
  def testBuild(self):
    batch_size = 3
    num_obs_dims = 5
    action_spec = tensor_spec.BoundedTensorSpec([1], tf.float32, 2.0, 3.0)
    ...
    self.assertAllEqual(
        actions.shape.as_list(), [batch_size] + action_spec.shape.as_list()
    )
  ...
  def testScalarAction(self):
    ...
    num_obs_dims = 5
    obs_spec = tensor_spec.TensorSpec([num_obs_dims], tf.float32)
    action_spec = tensor_spec.BoundedTensorSpec([], tf.float32, 2.0, 3.0)
  ...
  def test2DAction(self):
    ...
    num_obs_dims = 5
    obs_spec = tensor_spec.TensorSpec([num_obs_dims], tf.float32)
    action_spec = tensor_spec.BoundedTensorSpec([2, 3], tf.float32, 2.0, 3.0)
    actor_net = actor_network.ActorNetwork(obs_spec, action_spec)
  ...
  def testDictOfSingleAction(self):
    ...
    obs_spec = tensor_spec.TensorSpec([num_obs_dims], tf.float32)
    action_spec = {
        'motor': tensor_spec.BoundedTensorSpec([1], tf.float32, 2.0, 3.0)
    }
    actor_net = actor_network.ActorNetwork(obs_spec, action_spec)
