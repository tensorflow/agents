...
class GymWrapper(py_environment.PyEnvironment):
  ...
  def _step(self, action):
    ...
    # fix issue #283 -
    # some gym environments (e.g. FrozenLake-v0) use the action as index to transition
    # array. thus expects a hashable datatype.
    # when the action space is discrete, action is np.array scalar (e.g. array(2))
    # which is not hashable. thus, it has to be converted to hashable type (int):
