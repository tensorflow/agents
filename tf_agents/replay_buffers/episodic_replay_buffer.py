...
Episodes = collections.namedtuple(
    'Episodes', ['length', 'completed', 'tensor_lists']
)
...
class EpisodicReplayBuffer(replay_buffer_base.ReplayBuffer):
  ...
  def __init__(
    ...
    Args:
      ...
      completed_only: Scalar bool.  Whether to sample full episodes (in
        as_dataset if `num_steps = None`), and whether to sample subsets from
        full episodes (in as_dataset `num_steps != None`).
      buffer_size: How many episode IDs to precalculate in a buffer when using
        `as_dataset(..., single_deterministic_pass=False)`.  This parameter
        controls how often episode IDs are sampled according to their lengths in
        the `tf.data.Dataset` returned by `as_dataset`.  Choosing a small number
        means episodes are sampled more frequently, which is expensive, but new
        data in the replay buffer is seen more quickly by the resulting dataset.
        Choosing a larger number means the sampling is less frequent and less
        costly, but new episodes and updated episode lengths may not be
        respected for up to `buffer_size` requests to the `Dataset` iterator.
        For example, if `buffer_size > 1` then a new episode may be added to the
        replay buffer, but this episode won't be "seen" by the `Dataset` for up
        to `buffer_size - 1` more accesses.
      ...
      begin_episode_fn: A function that maps batched tensors respecting
        ...
        given entries are the start of a new episode.  Used by `add_batch`,
        `add_sequence`, and `extend_episodes` to indicate whether the
        `episode_id` should be incremented.  Default value: ```python
        begin_episode_fn = lambda traj: traj.is_first() ```
      end_episode_fn: A function that maps batched tensors respecting
        `data_spec` to a boolean scalar or vector Tensor, indicating whether the
        given entries are the end of an episode.  **NOTE** The current default
        behavior is to mark an episode as completed once it receives its final
        reward.  However, additional frames may be added to the episode after
        this. Typically, exactly one boundary frame (LAST -> FIRST), is likely
        to be added to the episode after it is marked as completed).  Used by
        `add_batch`, `add_sequence`, and `extend_episodes` to indicate whether
        to end the episode at `episode_id`.  Default value: ```python
      ...
      dataset_window_shift: Window shift used when calling `as_dataset` with
        ...
        `num_steps is not None`, then windows are created with a window overlap
        of `dataset_window_shift` and you will see each frame up to `num_steps`
        times.  For example, if `max_length=5`, `num_steps=2`,
    ...
    super(EpisodicReplayBuffer, self).__init__(
        data_spec, capacity, stateful_dataset=True
    )
  ...
  def create_episode_ids(self, num_episodes=None):
    ...
    Args:
      num_episodes: (Optional) int32, number of episode IDs to create. This may
        be a tensor.  If `None`, a scalar ID tensor is returned.
    ...
    if tf.is_tensor(num_episodes):
      if num_episodes.shape.rank != 0:
        raise ValueError(
            'num_episodes must be a scalar, but saw shape: {}'.format(
                num_episodes.shape
            )
        )
      return tf.fill(
    ...
    if num_episodes is not None and num_episodes > 0:
      if num_episodes > self._capacity:
        raise ValueError(
            'Buffer cannot create episode_ids when '
  ...
  def add_sequence(self, items, episode_id):
    ...
    with tf.device(self._device):
      with tf.name_scope('add_steps'):
        ...
        num_steps = tf.cast(
            tf.compat.dimension_value(item_0.shape[0])
            or tf.shape(input=item_0)[0],
            tf.int64,
        )
  ...
  def add_batch(self, items, episode_ids):
    ...
    Args:
      ...
      episode_ids: A int64 vector `Tensor` containing the ids of the episodes
        the items are being added to. Shaped `(num_episodes,)`.
    ...
    with tf.device(self._device):
      with tf.name_scope('add_batch'):
        ...
        begin_episode = self._begin_episode_fn(items)
        end_episode = self._end_episode_fn(items)
        batch_episode_ids = self._get_batch_episode_ids(
            episode_ids, begin_episode, end_episode
        )
  ...
  def _as_dataset(
    ...
    Args:
      ...
      num_steps: (Optional.) Scalar int.  How many contiguous frames to get per
        entry. Default is `None`: return full-length episodes.
      sequence_preprocess_fn: (Optional.) Preprocessing function for sequences
        ...
        batched.
      num_parallel_calls: Number of parallel calls to use in the dataset
        pipeline when extracting episodes.  Default is to have tensorflow
        determine the optimal number of calls.
      ...
      def _random_slice(flat_tensor_lists, id_, num_frames):
        ...
        flat = tuple(
            list_ops.tensor_list_gather(  # pylint: disable=g-complex-comprehension
                t,
                indices=tf.range(start_slice, end_slice),
                element_dtype=spec.dtype,
  ...
  def _single_deterministic_pass_dataset(
    ...
    Args:
      sample_batch_size: (Optional.) An optional batch_size to specify the
        ...
        the episodes may be different lengths and cannot be batched.
      ...
      num_parallel_calls: (Optional.) Number elements to process in parallel.
        See as_dataset() documentation.  Note, that the parallelism here is not
        "sloppy", in that setting this value does not affect the order in which
        frames are returned.
    ...
    ds = episode_ids_ds.map(read_episode, num_parallel_calls=num_parallel_calls)
    if sample_batch_size is None:
      ...
      ds_shards = tf.data.Dataset.range(sample_batch_size).map(
          lambda i: ds.shard(sample_batch_size, i)
      )
      ...
      def rebatch(ds_):
        def batch_nest(window):
          return tf.data.Dataset.zip(
              tf.nest.map_structure(
                  lambda d: d.batch(num_steps, drop_remainder=True), window
              )
          )
  ...
  def _clear(self, clear_all_variables=False):
    ...
    if clear_all_variables:
      ...
      assignments += [
          var.assign(_INVALID_EPISODE_ID * tf.ones_like(var))
  ...
  def _get_episode(self, episode_id):
    ...
    with tf.device(self._device), tf.name_scope('get_episode'):
      ...
      assert_valid = tf.Assert(
          episode_valid,
          [
              'Episode id',
              episode_id,
              'is not valid.  It points to location',
              episode_location,
              'but the episode at that location is currently id',
              episode_at_location,
          ],
      )
      ...
      def _end_episode_id():
        return tf.group(
            tf.compat.v1.scatter_update(
                self._episode_completed, [episode_location], 1
            )
        )

      episode_valid = tf.equal(
          self._episodes_loc_to_id_map.sparse_read(episode_location), episode_id
      )

      pred_value = end_episode & (episode_id >= 0) & episode_valid
      if pred_value.shape.rank != 0:
        raise ValueError(
            'Invalid condition shape: {} (should be scalar).'.format(
                pred_value.shape
            )
        )
      maybe_end = tf.cond(
  ...
  def _maybe_end_batch_episodes(self, batch_episode_ids, end_episode=False):
    ...
    def _execute():
      ...
      update_completed = tf.compat.v1.scatter_update(
          self._episode_completed, episodes_location_, 1
      )
  ...
  def _get_episode_id(self, episode_id, begin_episode=False, end_episode=False):
    ...
    def _assign_new_episode_id():
      ...
      update_completed = tf.compat.v1.scatter_update(
          self._episode_completed, episode_location, 0
      )
      reset_data = self._data_table.clear_rows(
          tf.expand_dims(episode_location, 0)
      )
      reset_length = tf.compat.v1.scatter_update(
          self._episode_lengths, episode_location, 0
      )
      with tf.control_dependencies(
    ...
    if pred_value.shape.rank != 0:
      raise ValueError(
          'Invalid condition predicate shape: {} (should be scalar).'.format(
              pred_value.shape
          )
      )
  ...
  def _get_batch_episode_ids(
    ...
    Args:
      ...
      begin_episode: A Boolean Tensor whether should increment each episode ID.
        ...
        batch_episode_ids.
      end_episode: A Boolean Tensor whether should end each episode ID. It can
        be a scalar or a vector with the same dimensions of batch_episode_ids.
    ...
    if batch_episode_ids.shape.rank != 1:
      raise ValueError('batch_episode_ids must be a vector with 1 dimension')
    ...
    def _update_batch_episode_ids():
      ...
      updated_batch_episode_ids = tf.compat.v1.where(
          ids_to_update_mask, scattered_updated_episode_ids, batch_episode_ids
      )
      episode_locations = tf.math.mod(new_batch_episode_ids, self._capacity)
      update_mapping = tf.compat.v1.scatter_update(
          self._episodes_loc_to_id_map,
          [episode_locations],
          [new_batch_episode_ids],
      )
      reset_completed = tf.compat.v1.scatter_update(
          self._episode_completed, [episode_locations], 0
      )
      reset_data = self._data_table.clear_rows(episode_locations)
      reset_length = tf.compat.v1.scatter_update(
          self._episode_lengths, episode_locations, 0
      )
      with tf.control_dependencies(
  ...
  def _sample_episode_ids(
    ...
    assert_nonempty = tf.compat.v1.assert_non_negative(
        last_id,
        message=(
            'EpisodicReplayBuffer is empty. Make sure to add items '
  ...
  def extract(self, locations, clear_data=False):
    ...
    Args:
      locations: A `1-D` Tensor of locations to extract from (note, this is NOT
        ...
        between `0` and `self.capacity`). Passing locations that are out of
        bounds will lead to runtime errors.
    ...
    def _extract_locked():
      ...
      if clear_data:
        with tf.control_dependencies(tf.nest.flatten(episodes)):
          ...
          clear_lengths = tf.compat.v1.scatter_update(
              self._episode_lengths, locations, 0
          )
          clear_completed = tf.compat.v1.scatter_update(
              self._episode_completed, locations, 0
          )
        with tf.control_dependencies(
  ...
  def extend_episodes(self, episode_ids, episode_ids_indices, episodes):
    ...
    Args:
      ...
      episode_ids_indices: An int64 vector containing the locations in
        `episode_ids` that are being extended.  Shaped `(num_episodes,)`, where
        `num_episodes <= max_num_episodes`.  Rows in `episodes` correspond to
        locations in `episode_ids_indices`.
    ...
    def _extend_locked(episode_ids, expanded_episode_ids):
      ...
      increment_lengths = self._increment_episode_length_locked(
          episode_locations, tf.gather(episodes.length, episode_valid_idx)
      )
      ...
      extend = self._data_table.extend(
          episode_locations,
          tf.nest.map_structure(
              lambda tl: tf.gather(tl, episode_valid_idx), episodes.tensor_lists
          ),
      )
    ...
    with tf.device(self._device):
      with tf.name_scope('extend'):
        ...
        # of episode_ids).
        expanded_begin_episode = tf.scatter_nd(
            column_indices, begin_episode, episode_ids_shape
        )
        expanded_mask = tf.scatter_nd(
            column_indices,
