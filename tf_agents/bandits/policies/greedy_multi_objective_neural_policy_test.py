...
class HeteroscedasticDummyNet(
  ...
  def __init__(self, kernel_weights: np.ndarray, bias: np.ndarray):
    ...
    action_spec = array_spec.BoundedArraySpec(
        [1], np.float32, 1, kernel_weights.shape[1]
    )

    input_tensor_spec = tensor_spec.from_spec(input_spec)
    action_tensor_spec = tensor_spec.from_spec(action_spec)

    super(HeteroscedasticDummyNet, self).__init__(
        input_tensor_spec, action_tensor_spec
    )
...
class ScalarizeObjectivesTest(test_utils.TestCase):
  ...
  def testScalarizeObjectives(self):
    ...
    self.assertAllClose(
        self.evaluate(scalarized_reward),
        [[3, 4], [9, 10]],
        rtol=1e-4,
        atol=1e-3,
    )
...
class GreedyRewardPredictionPolicyTest(test_utils.TestCase):
  ...
  def _create_arm_policy_and_observations(
    ...
    policy = greedy_multi_objective_policy.GreedyMultiObjectiveNeuralPolicy(
        ...
        emit_policy_info=(
            ...
            policy_utilities.InfoFields.MULTIOBJECTIVE_SCALARIZED_PREDICTED_REWARDS_MEAN,
        ),
    )
    ...
    policy.scalarizer.set_parameters(
        ...
        transform_params={
            multi_objective_scalarizer.HyperVolumeScalarizer.SLOPE_KEY: (
                tf.constant(
                    [[0.2, 0.2, 0.2], [0.1, 0.1, 0.1]], dtype=tf.float32
                )
            ),
  ...
  def testPredictedRewards(self):
    ...
    predicted_rewards_expected_array = np.array([
        [[8, 11, 14], [12, 8, 13], [11, 14, 8]],
        [[5, 8, 11], [10, 5, 9], [8, 11, 5]],
    ])
  ...
  def testPerArmRewards(self):
    ...
    self.assertAllEqual(
        chosen_arm_features[0], first_arm_features[first_action]
    )
    ...
    self.assertAllEqual(
        predicted_rewards[:, :, 0],
        getattr(
            padded_p_info, policy_utilities.InfoFields.PREDICTED_REWARDS_MEAN
        )[:, :, 0],
    )
    self.assertAllEqual(
  ...
  def testPerArmRewardsSparseObs(self):
    ...
    first_action = action[0]
    self.assertAllEqual(
        chosen_arm_features['name'][0], first_arm_name_feature[first_action]
    )
