...
class NeuralLinUCBPolicyTest(parameterized.TestCase, test_utils.TestCase):
  ...
  def _b(self):
    return [
        tf.constant(r * np.ones(self._encoding_dim), dtype=tf.float32)
  ...
  def _b_numpy(self):
    return [
        np.array([r * np.ones(self._encoding_dim)], dtype=np.float32)
        for r in range(self._num_actions)
    ]
    ...
    return ts.TimeStep(
        ...
        tf.constant(1.0, dtype=tf.float32, shape=[batch_size], name='discount'),
        {
            ...
            bandit_spec_utils.PER_ARM_FEATURE_KEY: tf.constant(
                np.array(range(batch_size * self._num_actions * arm_obs_dim)),
                dtype=tf.float32,
                shape=[batch_size, self._num_actions, arm_obs_dim],
                name='observation',
            ),
            bandit_spec_utils.NUM_ACTIONS_FEATURE_KEY: tf.ones(
  ...
  def _get_predicted_rewards_from_linucb(self, observation_numpy, batch_size):
    ...
    predicted_rewards_array = np.stack(predicted_rewards, axis=-1).reshape(
  ...
  def testActionBatchWithVariablesAndPolicyUpdate(
    ...
    for k in range(1, self._num_actions + 1):
      a_initial_value = (
          k + 1 + 2 * k * tf.eye(self._encoding_dim, dtype=tf.float32)
      )
  ...
  def testPredictedRewards(self, batch_size, actions_from_reward_layer):
    ...
    self.assertAllClose(
        p_info.predicted_rewards_mean, predicted_rewards_expected
    )
  ...
  def testSparseObs(self, batch_size, actions_from_reward_layer):
    ...
    observations = {
        'global': {'sport': tf.constant(['snooker', 'chess'])},
        'per_arm': {
            'name': tf.constant(
                [['george', 'george', 'george'], ['bob', 'bob', 'bob']]
            ),
            'fruit': tf.constant(
                [['banana', 'banana', 'banana'], ['kiwi', 'kiwi', 'kiwi']]
            ),
        },
    }
    ...
    self.assertAllEqual(
        p_info.chosen_arm_features['name'][0],
