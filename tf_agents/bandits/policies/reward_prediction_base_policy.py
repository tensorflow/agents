...
class RewardPredictionBasePolicy(tf_policy.TFPolicy):
  ...
  def _action(
    ...
    """
    step = super(RewardPredictionBasePolicy, self)._action(
  ...
  def _distribution(self, time_step, policy_state):
    ...
    if isinstance(
        self._reward_network, heteroscedastic_q_network.HeteroscedasticQNetwork
    ):
    ...
    if (
        predicted_reward_values.shape[-1] is not None
        and predicted_reward_values.shape[-1] != self._expected_num_actions
    ):
      raise ValueError(
          'The number of actions ({}) does not match the reward_network output'
          ' size ({}).'.format(
              self._expected_num_actions, predicted_reward_values.shape[1]
          )
      )
    ...
    return policy_step.PolicyStep(
        action_distribution, policy_state, policy_info
    )
