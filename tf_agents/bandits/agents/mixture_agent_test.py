...
class WeightRotatingMixtureAgent(mixture_agent.MixtureAgent):
  ...
  def _update_mixture_distribution(self, experience):
    ...
    new_values = tf.concat([weight_values[1:], weight_values[0:1]], 0)
...
def _get_action_step(action, num_agents, num_actions):
  ...
  return policy_step.PolicyStep(
      ...
      info={
          mixture_policy.MIXTURE_AGENT_ID: choices,
          mixture_policy.SUBPOLICY_INFO: policy_utilities.PolicyInfo(
...
class MixtureAgentTest(test_utils.TestCase, parameterized.TestCase):
  ...
  def testAgentWithDifferentSubagentsUpdate(self):
    ...
    dist = tfd.Categorical(probs=tf.Variable([0.0, 1.0]))
    mixed_agent = WeightRotatingMixtureAgent(dist, agents)
    initial_step, final_step = _get_initial_and_final_steps(
        batch_size, context_dim
    )
  ...
  @test_cases()
  def testDynamicPartitionOfNestedTensors(
      self, batch_size, context_dim, num_agents
  ):
    tensor1 = tf.reshape(
    ...
    self.assertAllEqual(
        evaluated[0][0]['a'].shape, [(batch_size + 1) // 2, context_dim]
    )
    self.assertAllEqual(
        evaluated[1][0]['a'].shape, [batch_size // 2, context_dim]
    )
    self.assertAllEqual(
        evaluated[0][1].shape, [(batch_size + 1) // 2, num_agents]
    )
