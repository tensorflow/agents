...
class NeuralFalconAgentTest(tf.test.TestCase, parameterized.TestCase):
  ...
  def _create_agent(
    ...
    if accepts_per_arm_features:
      ...
      reward_net = global_and_arm_feature_network.create_feed_forward_dot_product_network(
          obs_spec,
          global_layers=[3],
          arm_layers=[3],
  ...
  def _generate_observations(
      self, batch_size: int, accepts_per_arm_features: bool
  ) -> types.NestedTensor:
    if accepts_per_arm_features:
      num_actions = self._num_actions(accepts_per_arm_features)
      observations = {
          bandit_spec_utils.GLOBAL_FEATURE_KEY: np.array(
              np.random.normal(size=[batch_size, 3]), dtype=np.float32
          ),
          bandit_spec_utils.PER_ARM_FEATURE_KEY: np.array(
              np.random.normal(size=[batch_size, num_actions, 3]),
              dtype=np.float32,
          ),
          bandit_spec_utils.NUM_ACTIONS_FEATURE_KEY: num_actions * tf.ones(
  ...
  def _get_policy_info(
    ...
    num_actions = self._num_actions(accepts_per_arm_features)
    dummy_predicted_rewards = tf.ones(
        [batch_size, num_actions], dtype=tf.float32
    )
    if accepts_per_arm_features:
  ...
  def _generate_training_experience(
      self, accepts_per_arm_features: bool
  ) -> types.NestedTensor:
    batch_size = 240
    ...
    rewards = self._generate_reward(
        observations, actions, accepts_per_arm_features
    )
    ...
    experience = tf.nest.map_structure(
        lambda t: tf.expand_dims(t, axis=1), experience
    )
    return experience
  ...
  def testTrainedPolicy(self, accepts_per_arm_features):
    ...
    for _ in range(80):
      self.evaluate(
          agent.train(
              self._generate_training_experience(accepts_per_arm_features), None
          ).loss
      )
    ...
    self.assertAllEqual(
        actions, np.argmax(p_info.predicted_rewards_mean, axis=1)
    )
