...
class TrainerTest(tf.test.TestCase, parameterized.TestCase):
  ...
  @parameterized.named_parameters(
      dict(
          testcase_name='_stat_stoch__linucb',
          environment_name='stationary_stochastic',
          agent_name='LinUCB',
      ),
      dict(
          testcase_name='_stat_stoch__lints',
          environment_name='stationary_stochastic',
          agent_name='LinTS',
      ),
      dict(
          testcase_name='_stat_stoch__epsgreedy',
          environment_name='stationary_stochastic',
          agent_name='epsGreedy',
      ),
      dict(
          testcase_name='_wheel__linucb',
          environment_name='wheel',
          agent_name='LinUCB',
      ),
      dict(
          testcase_name='_wheel__lints',
          environment_name='wheel',
          agent_name='LinTS',
      ),
      dict(
          testcase_name='_wheel__epsgreedy',
          environment_name='wheel',
          agent_name='epsGreedy',
      ),
      dict(
          testcase_name='_wheel__mix',
          environment_name='wheel',
          agent_name='mix',
      ),
  )
  def testAgentAndEnvironmentRuns(self, environment_name, agent_name):
    ...
    agent = trainer_test_utils.get_agent_by_name(
        agent_name, environment.time_step_spec(), environment.action_spec()
    )
    ...
    with mock.patch.object(
        export_utils, 'logging', new_callable=MockLog
    ) as mock_logging:
      trainer.train(
          root_dir=tempfile.mkdtemp(dir=os.getenv('TEST_TMPDIR')),
          agent=agent,
