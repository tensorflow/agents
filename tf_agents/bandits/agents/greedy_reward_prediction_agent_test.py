...
def test_cases():
  return parameterized.named_parameters(
      ...
      {
          'testcase_name': 'EpsilonGreedy',
          'agent_class': functools.partial(
              neural_epsilon_greedy_agent.NeuralEpsilonGreedyAgent, epsilon=0.05
          ),
          'use_num_samples_list': False,
      },
  )
...
class AgentTest(tf.test.TestCase, parameterized.TestCase):
  ...
  @test_cases()
  def testTrainAgentWithMaskAndConstraint(
      self, agent_class, use_num_samples_list
  ):
    reward_net = DummyNet(self._observation_spec, self._action_spec)
    optimizer = tf.compat.v1.train.GradientDescentOptimizer(learning_rate=0.1)
    reward_spec = {
  ...
  @test_cases()
  def testTrainAgentWithLaplacianSmoothingInvalidMatrix(
      self, agent_class, use_num_samples_list
  ):
    if tf.executing_eagerly:
      return
  ...
  @test_cases()
  def testTrainPerArmAgentWithConstraint(
      self, agent_class, use_num_samples_list
  ):
