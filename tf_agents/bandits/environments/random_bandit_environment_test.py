...
class RandomBanditEnvironmentTest(tf.test.TestCase, parameterized.TestCase):
  ...
  @parameterized.parameters(
      dict(overall_observation_shape=[3, 4, 5, 6], batch_dims=2),
      dict(overall_observation_shape=[3, 3, 3, 3], batch_dims=0),
  )
  def testInvalidObservationBatchShape(
    ...
    observation_distribution = tfd.Independent(
        tfd.Normal(
            tf.zeros(overall_observation_shape),
  ...
  @parameterized.parameters(
      dict(overall_reward_shape=[3, 4, 5, 6], batch_dims=2),
      dict(overall_reward_shape=[4, 5, 6], batch_dims=0),
  )
  def testInvalidRewardBatchShape(self, overall_reward_shape, batch_dims):
    observation_distribution = tfd.Normal(
        tf.zeros(overall_reward_shape[0]), tf.ones(overall_reward_shape[0])
    )
  ...
  @parameterized.parameters(
      dict(overall_observation_shape=[4, 5, 6], overall_reward_shape=[3]),
      dict(overall_observation_shape=[3], overall_reward_shape=[1]),
  )
  ...
  @parameterized.named_parameters(
      dict(
          testcase_name='_observation_[]_action_[]_batch_1',
          observation_shape=[],
          action_shape=[],
          batch_size=1,
      ),
      dict(
          testcase_name='_observation_[3, 4, 5, 6]_action_[2, 3, 4]_batch_32',
  ...
  def testObservationAndRewardShapes(
    ...
    self.assertAllEqual(expected_reward_shape, self.evaluate(tf.shape(reward)))
