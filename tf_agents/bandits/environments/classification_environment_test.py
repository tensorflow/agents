...
def deterministic_reward_distribution(reward_table):
  ...
  return tfd.Independent(
      tfd.Deterministic(loc=reward_table), reinterpreted_batch_ndims=2
  )
...
class ClassificationEnvironmentTest(tf.test.TestCase, parameterized.TestCase):
  ...
  def testObservationShapeAndValue(self, context, labels, batch_size):
    ...
    for _ in range(100):
      ...
      np.testing.assert_array_equal(
          observation.shape, expected_observation_shape
      )
  ...
  def testReturnsCorrectRewards(self):
    ...
    for _ in range(10):
      ...
      np.testing.assert_almost_equal(
          self.evaluate(reward), self.evaluate(tf.ones_like(reward))
      )
  ...
  def testPreviousLabelIsSetCorrectly(self):
    ...
    if tf.executing_eagerly():
      np.testing.assert_almost_equal(
          self.evaluate(time_step_label), self.evaluate(env._previous_label)
      )
      np.testing.assert_almost_equal(
          self.evaluate(next_time_step_label), self.evaluate(env._current_label)
      )
    else:
      with self.cached_session() as sess:
        time_step_label_value, next_time_step_label_value = sess.run(
            [time_step_label, next_time_step_label]
        )
        ...
        np.testing.assert_almost_equal(
            next_time_step_label_value, current_label_value
        )
  ...
  def testShuffle(self):
    ...
    dataset.shuffle = mock.Mock(
        spec=dataset.shuffle, side_effect=dataset.shuffle
    )
    ce.ClassificationBanditEnvironment(dataset, reward_distribution, batch_size)
  ...
  def testPrefetch(self, mock_dataset_iterator):
    ...
    mock_prefetch = mock.Mock(
        spec=dataset.prefetch, return_value=batched_dataset
    )
    ...
    dataset.batch = mock.Mock(
        spec=batched_dataset, return_value=batched_dataset
    )
