...
class RankingPyEnvironment(bandit_py_environment.BanditPyEnvironment):
  ...
  def __init__(
    ...
    Args:
      ...
      item_sampling_fn: A function that outputs a random 1 array or list of ints
        or floats (same type as the output of `global_context_sampling_fn`).
        This output is the per-arm context. Its shape must be consistent across
        calls.
      ...
      feedback_model: The type of feedback model. Currently only implemented is
        ...
        of the chosen item, and `v` is the value of the choice.
      click_model: The way the environment models that diversity is desired. --
        `ghost_actions`: For every dimension of the item space, a unit vector is
        added to the list of available items. If one of these unit-vector items
        gets selected, it results in a `no-click`. -- `distance_based`:
        Inner-product scores are calculated, and if none of the scores exceed a
        given parameter, no item is clicked.
  ...
  def _apply_action(self, action: np.ndarray) -> types.Array:
    ...
    if self._click_model == ClickModel.GHOST_ACTIONS:
      ...
      chosen_items = self._choose_items_distance_based(
          global_obs, slotted_items
      )
    else:
      raise NotImplementedError(
          'Diversity model {} not implemented'.format(self._click_model)
      )
  ...
  def _step(self, action):
    ...
    new_reward = tf.nest.map_structure(
        lambda x, t: x.astype(t), reward, self.reward_spec()
    )
    return ts.TimeStep(
  ...
  def _choose_items_distance_based(self, global_obs, slotted_items):
    ...
    scores = np.concatenate(
        [scores, np.ones([self._batch_size, 1]) * self._distance_threshold],
        axis=1,
    )
...
class ExplicitPositionalBiasRankingEnvironment(
  ...
  def _get_relevances(self, global_obs, slotted_items):
    ...
    clipped_relevances = np.clip(relevances, 0.0, 1.0)
