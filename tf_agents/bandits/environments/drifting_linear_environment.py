...
def _update_row(input_x, updates, row_index):
  ...
  indices = tf.concat(
      [
          ...
          tf.reshape(tf.range(n, dtype=tf.int32), [n, 1]),
      ],
      axis=-1,
  )
...
class DriftingLinearDynamics(nsse.EnvironmentDynamics):
  ...
  def __init__(
    ...
    Args:
      ...
      drift_distribution: A scalar distribution from `tfp.distributions` of type
    ...
    if observation_to_reward_shape != expected_observation_to_reward_shape:
      raise ValueError(
          'Observation to reward has {} as expected shape; got {}'.format(
              expected_observation_to_reward_shape, observation_to_reward_shape
          )
      )
  ...
  def reward(
    ...
    reward = (
        tf.matmul(observation, self._current_observation_to_reward)
        + self._current_additive_reward
    )
    return reward
