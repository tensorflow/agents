...
def compute_optimal_action(
    observation: np.ndarray, delta: float
) -> types.Array:
...
def compute_optimal_reward(
    observation: np.ndarray, delta: float, mu_inside: float, mu_high: float
) -> types.Array:
...
class WheelPyEnvironment(bandit_py_environment.BanditPyEnvironment):
  ...
  def __init__(
    ...
    Args:
      ...
      mu_high: (float) Mean reward for the optimal action if the context norm is
        above delta.
    ...
    if self._mu_base.shape != (5,):
      raise ValueError(
          "The length of 'mu_base' must be 5, but saw 'mu_base': %s"
          % (self._mu_base,)
      )
    ...
    if self._mu_base[0] != max(self._mu_base):
      raise ValueError(
          'The first action in mu_base should have the highest '
  ...
  def _reward_fn(self, observation, action):
    ...
    reward_outside = (1.0 - is_norm_below_delta) * r_outside[
        np.arange(self._batch_size), action
    ]
