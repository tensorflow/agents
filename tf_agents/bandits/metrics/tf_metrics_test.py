...
class SimpleThresholdConstraint(constraints.BaseConstraint):
  ...
  def __init__(
      self, time_step_spec, action_spec, batch_size, threshold, name=None
  ):
...
class TFMetricsTest(parameterized.TestCase, tf.test.TestCase):
  ...
  @parameterized.named_parameters(
      (
          'RegretMetricName',
          tf_metrics.RegretMetric,
          compute_optimal_reward,
          'RegretMetric',
      ),
      (
          'SuboptimalArmsMetricName',
          tf_metrics.SuboptimalArmsMetric,
  ...
  @parameterized.named_parameters([
      ('TestRegret', tf_metrics.RegretMetric, compute_optimal_reward, 9),
      (
          'TestSuboptimalArms',
          tf_metrics.SuboptimalArmsMetric,
          compute_optimal_action,
  ...
  def testRegretMetricBatched(
      self, metric_class, fn, batch_size, expected_result
  ):
  ...
  def testRegretMetricWithRewardDict(
      self,
      metric_class=tf_metrics.RegretMetric,
      fn=compute_optimal_reward,
      batch_size=8,
      expected_result=6.5,
  ):
  ...
  def testConstraintViolationMetric(
    ...
    stc = SimpleThresholdConstraint(
        time_step_spec, action_spec, batch_size=batch_size, threshold=threshold
    )
