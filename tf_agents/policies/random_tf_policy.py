...
class RandomTFPolicy(tf_policy.TFPolicy):
  ...
  def __init__(
    ...
    observation_and_action_constraint_splitter = kwargs.get(
        'observation_and_action_constraint_splitter', None
    )
    self._accepts_per_arm_features = kwargs.pop(
    ...
    if self._stationary_mask is not None:
      ...
      assert (
          action_spec.dtype.is_integer
      ), 'To use a stationary mask, action dtype must be integer.'
      ...
      assert self._stationary_mask.shape[-1] == num_actions, (
          ...
          'but we get {} and {}.'.format(
              num_actions, self._stationary_mask.shape[-1]
          )
      )
  ...
  def _action(self, time_step, policy_state, seed):  # pytype: disable=signature-mismatch  # overriding-parameter-count-checks
    ...
    if observation_and_action_constraint_splitter is not None:
      ...
      action_ = tf.cast(
          masked_categorical.sample() + action_spec.minimum, action_spec.dtype
      )
    ...
    if time_step is not None:
      with tf.control_dependencies(
          tf.nest.flatten(
              tf.nest.map_structure(_maybe_convert_sparse_tensor, time_step)
          )
      ):
