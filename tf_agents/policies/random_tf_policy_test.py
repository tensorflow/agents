...
class RandomTFPolicyTest(test_utils.TestCase, parameterized.TestCase):
    ...
    if use_per_arm_features:
      ...
      observation = {
          ...
          bandit_spec_utils.PER_ARM_FEATURE_KEY: tf.constant(
              [[5, 6], [7, 8], [9, 10], [11, 12]], tf.float32
          ),
          bandit_spec_utils.NUM_ACTIONS_FEATURE_KEY: tf.constant(
              num_arms, tf.int32
          ),
      }
  ...
  def testEmitLogProbability(self, dtype):
    ...
    p = 1.0 / 21 if dtype.is_integer else 1.0 / 20
    np.testing.assert_allclose(
        np.array(step.info.log_probability, dtype=np.float32),
        np.array(
            np.log([
                [math.pow(p, 6) for _ in range(3)],
  ...
  def testMasking(self, dtype):
    ...
    self.assertAllClose(
        ...
        tf.constant(np.log(1.0 / 3), shape=[batch_size]),
    )
  ...
  def testStationaryMask(self, dtype):
    ...
    self.assertTrue(np.all(action_ <= 5))
    self.assertAllEqual(
        np.array(mask)[action_ - action_spec.minimum], np.ones([batch_size])
    )
  ...
  def testBothDynamicAndStationaryMask(self, dtype):
    ...
    self.assertTrue(np.all(action_ <= 5))
    self.assertAllEqual(
        np.array(intersect_mask)[action_ - action_spec.minimum],
        np.ones([batch_size]),
    )
    ...
    self.assertAllClose(
        step.info.log_probability,
        tf.constant(np.log(1.0 / 2), shape=[batch_size]),
    )
  ...
  def testNumActions(self, dtype):
    ...
    self.assertAllClose(
        ...
        tf.constant(np.log(1.0 / 2), shape=[half_batch_size]),
    )
    self.assertAllClose(
        step.info.log_probability[half_batch_size:],
        tf.constant(np.log(1.0 / 3), shape=[half_batch_size]),
    )
  ...
  def testInfoSpec(self, dtype):
    ...
    self.assertEqual(
        ...
        batched_action_step.action[0].shape,
    )
    self.assertEqual(
        (
            batch_size,
        ...
        batched_action_step.action[1].shape,
    )
    ...
    self.assertEqual(
        ...
        batched_action_step.info[0].shape,
    )
    self.assertEqual(
        (
            batch_size,
        ...
        batched_action_step.info[1].shape,
    )
