...
class CEMPolicy(tf_policy.TFPolicy):
  ...
  def __init__(
    ...
    Args:
      ...
      preprocess_state_action: The shape of state is (B, ...) and the shape of
        ...
        will be tile_batched to be (BxN, ...) and the action will be reshaped to
        be (BxN, A). When preprocess_state_action is not enabled, the same
  ...
  def _initial_params(
    ...
    mean = tf.nest.map_structure(
        broadcast_to_batch, convert_nest_lists_to_np_array(self._init_mean)
    )
    var = tf.nest.map_structure(
        broadcast_to_batch, convert_nest_lists_to_np_array(self._init_var)
    )
    ...
    def body(
      ...
      Args:
        mean: A [B, A] sized tensor, where, a is the size of the action space,
          ...
          variance value of the sample distribution
      ...
      if outer_rank == 2:
        scores, next_policy_state = self._score_with_time(
            observation, actions, step_type, policy_state, seq_size
        )  # [BxT, N]
      else:
        scores, next_policy_state = self._score(
            observation, actions, step_type, policy_state
        )  # [B, N]
  ...
  def compute_target_q(
    ...
    scores, next_policy_state = self._q_network(
        (observation, action),
        step_type=step_type,
  ...
  def _score(
    ...
    scores, next_policy_state = self.compute_target_q(
        observation, sample_actions, step_type, policy_state
    )  # [BxN]
    ...
    if self._preprocess_state_action:
      next_policy_state = tf.nest.map_structure(
          lambda x: tf.reshape(
              x,
              [-1, self._num_samples] + x.shape.as_list()[1:],  # pylint:disable=g-long-lambda
          )[:, 0, ...],
          next_policy_state,
      )
  ...
  def _distribution(
    ...
    if not policy_state and self._q_network and self._q_network.state_spec:
      batch_size = nest_utils.get_outer_shape(
          time_step.observation, self._observation_spec
      )[0]
    ...
    if self._actor_policy is not None:
      ...
      def select_best_action_consider_actor(action1, action2):
        ...
        return action1 * use_cem_expanded + action2 * (
            tf.ones_like(use_cem_expanded, action1.dtype) - use_cem_expanded
        )

      best_action_consider_actor = tf.nest.map_structure(
          select_best_action_consider_actor,
          best_action_consider_actor,
          potential_best_action,
      )

    distribution = tf.nest.map_structure(
        tfp.distributions.Deterministic, best_action_consider_actor
    )
    if self._info_spec and 'target_q' in self._info_spec:
      batch_size = nest_utils.get_outer_shape(time_step, self._time_step_spec)[
          0
      ]
