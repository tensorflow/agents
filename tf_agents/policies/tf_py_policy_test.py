...
class TFPyPolicyTest(test_utils.TestCase):
  ...
  def testRandomPyPolicyGeneratesActionTensors(self):
    ...
    tf_py_random_policy = tf_py_policy.TFPyPolicy(
        random_py_policy.RandomPyPolicy(
            time_step_spec=time_step_spec, action_spec=array_action_spec
        )
    )
  ...
  def testAction(self):
    ...
    expected_py_policy_state = np.ones(
        py_policy_state_spec.shape, py_policy_state_spec.dtype
    )
    expected_py_time_step = tf.nest.map_structure(
        lambda arr_spec: np.ones((1,) + arr_spec.shape, arr_spec.dtype),
        py_time_step_spec,
    )
    expected_py_action = np.ones(
        (1,) + py_action_spec.shape, py_action_spec.dtype
    )
    expected_new_py_policy_state = np.zeros(
        py_policy_state_spec.shape, py_policy_state_spec.dtype
    )
    expected_py_info = np.zeros(
        py_policy_info_spec.shape, py_policy_info_spec.dtype
    )
  ...
  def testZeroState(self):
    ...
    py_policy_state_spec = array_spec.BoundedArraySpec(
        (policy_state_length,), np.int32, 1, 1
    )
  ...
  def testPyPolicyIsBatchedTrue(self):
    ...
    observation_dims = 3
    batch_size = 2
    array_action_spec = array_spec.BoundedArraySpec(
        (action_dims,), np.int32, -10, 10
    )
    ...
    tf_py_random_policy = tf_py_policy.TFPyPolicy(
        random_py_policy.RandomPyPolicy(
            time_step_spec=array_time_step_spec, action_spec=array_action_spec
        ),
