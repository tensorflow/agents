...
<meta itemprop="name" content="tf_agents.networks.value_rnn_network.ValueRnnNetwork" />
...
*   <b>`input_dropout_layer_params`</b>: Optional list of dropout layer
    parameters, where each item is the fraction of input units to drop. The
    dropout layers are interleaved with the fully connected layers; there is a
    dropout layer after each fully connected layer, except if the entry in the
    list is None. This list must have the same length of input_fc_layer_params,
    or be None.
...
* <b>`ValueError`</b>: If `observation_spec` contains more than one observation.
