...
<meta itemprop="name" content="tf_agents.networks.actor_distribution_network.ActorDistributionNetwork" />
...
*   <b>`activation_fn`</b>: Activation function, e.g. tf.nn.relu,
    slim.leaky_relu, ...
*   <b>`discrete_projection_net`</b>: Callable that generates a discrete
    projection network to be called with some hidden state and the outer_rank of
    the state.
*   <b>`continuous_projection_net`</b>: Callable that generates a continuous
    projection network to be called with some hidden state and the outer_rank of
    the state.
