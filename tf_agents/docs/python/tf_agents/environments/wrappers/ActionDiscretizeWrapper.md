...
<meta itemprop="name" content="tf_agents.environments.wrappers.ActionDiscretizeWrapper" />
...
* <b>`num_actions`</b>: A np.array of the same shape as the environment's
    action_spec. Elements in the array specify the number of actions to
    discretize to for each dimension.

#### Raises:

* <b>`ValueError`</b>: IF the action_spec shape and the limits shape are not equal.
