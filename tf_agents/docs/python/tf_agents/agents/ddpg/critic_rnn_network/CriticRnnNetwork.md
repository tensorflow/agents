...
<meta itemprop="name" content="tf_agents.agents.ddpg.critic_rnn_network.CriticRnnNetwork" />
...
* <b>`observation_fc_layer_params`</b>: Optional list of fully_connected parameters,
    where each item is the number of units in the layer. This is applied
    after the observation convultional layer.
* <b>`action_fc_layer_params`</b>: Optional list of parameters for a fully_connected
    layer to apply to the actions, where each item is the number of units
    in the layer.
* <b>`joint_fc_layer_params`</b>: Optional list of parameters for a fully_connected
