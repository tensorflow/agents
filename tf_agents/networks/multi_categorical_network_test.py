...
class MultiCategoricalNetworkTest(tf.test.TestCase):

  def testBuild(self):
    output_spec = [
      tensor_spec.BoundedTensorSpec([], tf.int32, 0, 1),
      tensor_spec.BoundedTensorSpec([], tf.int32, 0, 4)]
    network = multi_categorical_network.MultiCategoricalNetwork(
      output_spec)
    ...
    self.assertEqual(multi_categorical_network.MultiCategoricalDistributionBlock,
      type(distribution))
    ...
    self.assertAllInRange(sample[:, 0], 0, 1)
    self.assertAllInRange(sample[:, 1], 0, 4)
