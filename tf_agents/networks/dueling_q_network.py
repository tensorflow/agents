...
class DuelingQNetwork(network.Network):
  ...
  def __init__(self,
    ...
    """Creates an instance of `DuelingQNetwork`.
    Args:
    ...
    fc_value_layer = tf.keras.layers.Dense(
        1,
        activation=None,
    ...
    fc_advantage_layer = tf.keras.layers.Dense(
        num_actions,
        activation=None,
        kernel_initializer=tf.compat.v1.initializers.random_uniform(
            minval=-0.03, maxval=0.03),
    ...
    self._fc_value_layer = fc_value_layer
    self._fc_advantage_layer = fc_advantage_layer
