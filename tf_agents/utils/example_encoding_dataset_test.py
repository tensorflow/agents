...
class TFRecordsUtilsTest(test_utils.TestCase):
  ...
  def setUp(self):
    ...
    self.dataset_path = os.path.join(
        self.get_temp_dir(), "test_tfrecord_dataset.tfrecord"
    )
  ...
  def test_load_tfrecord_dataset(self):
    ...
    example_encoding_dataset.load_tfrecord_dataset(
        [self.dataset_path], buffer_size=2
    )
    example_encoding_dataset.load_tfrecord_dataset(
        [self.dataset_path], buffer_size=2, as_experience=True
    )
  ...
  def test_conflicting_specs(self):
    ...
    example_encoding_dataset.encode_spec_to_file(
        self.other_dataset_path, self.other_data_spec
    )
    with self.assertRaises(IOError):
      example_encoding_dataset.load_tfrecord_dataset(
  ...
  def test_with_py_driver(self):
    ...
    trajectory_spec = trajectory.from_transition(
        env.time_step_spec(), policy.policy_step_spec, env.time_step_spec()
    )
